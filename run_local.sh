...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "facesnet"; then
			verbosePrint "Starting docker network facesnet on 10.147.79.0/24"
			docker network create -d bridge --subnet 10.147.79.0/24 --gateway 10.147.79.1 "facesnet"
		else
			verbosePrint "Docker network facesnet already running"
		fi
	fi
}
...
if [ -z "$1" ]; then
	...
	ALL_COMPONENTS=(SOURCE MYSQL TOMEE)
	for comp in "$@"; do
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="c8f0a85c108601cba53d4c3e240da9ee"
...
if [ "$START_MYSQL" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_MYSQL" == "docker" ]; then
		...
		if [ ! -f ".mysqlPid" ]; then
			...
			verbosePrint "docker run --rm -d -p 3306:3306  -e MYSQL_ALLOW_EMPTY_PASSWORD=true   --net=facesnet --name=mysql   mysql:$TYPE_SOURCE_MYSQL_VERSION"
	...
	mysql -uroot --protocol=tcp -NB -e "create database if not exists faces"

	mysql -uroot --protocol=tcp faces <src/db/init-ddl.sql

	mysql -uroot --protocol=tcp faces <src/db/init-data.sql
...
if [ "$START_TOMEE" = "YES" ]; then
	...
	f_deploy() {
		cp target/faces.war $targetPath
	}
	f_deploy
	...
	if [ "$TYPE_SOURCE_TOMEE" == "docker" ]; then
		...
		if [ ! -f ".tomeePid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080  --net=facesnet --name=tomee $ADD_HOST_INTERNAL  -e JAVA_OPTS="-Djdbc/facesdatabase.JdbcUrl=jdbc:mysql://$REPLVAR_TOMEE_MYSQL/faces?useSSL=false" -v "$(pwd)/localrun/webapps":/usr/local/tomee/webapps tomee:$TYPE_SOURCE_TOMEE_VERSION"
			dockerContainerIDtomee=$(docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 \
				--net=facesnet \
				--name=tomee $ADD_HOST_INTERNAL \
				-e JAVA_OPTS="-Djdbc/facesdatabase.JdbcUrl=jdbc:mysql://$REPLVAR_TOMEE_MYSQL/faces?useSSL=false" \
...
if [ "$TAIL" == "YES" ]; then
	echo "http://localhost:8080/faces"
